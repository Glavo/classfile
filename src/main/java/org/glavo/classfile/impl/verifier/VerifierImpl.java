...
import static org.glavo.classfile.impl.verifier.VerificationSignature.BasicType.*;
import org.glavo.classfile.impl.verifier.VerificationSignature.BasicType;

import static org.glavo.classfile.impl.verifier.VerificationFrame.FLAG_THIS_UNINIT;
...
static final char JVM_SIGNATURE_SPECIAL = '<',
    ...
    private byte[] generate_code_data(ByteBuffer code, int code_length) {
        ...
        var bcs = new RawBytecodeHelper(code);
        while (!bcs.isLastBytecode()) {
            if (bcs.rawNext() != RawBytecodeHelper.ILLEGAL) {
                int bci = bcs.bci;
    ...
    boolean verify_invoke_init(RawBytecodeHelper bcs, int ref_class_index, VerificationType ref_class_type,
                               ...
                               boolean this_uninit, VerificationWrapper.ConstantPoolWrapper cp, VerificationTable stackmap_table) {
        int bci = bcs.bci;
